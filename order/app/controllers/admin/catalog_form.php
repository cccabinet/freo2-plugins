...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['view']) && $_POST['view'] === 'preview') {
        ...
        if (isset($_POST['_type']) && $_POST['_type'] === 'json') {
            ...
            if (empty($warnings)) {
                ...
                // フォワード
                forward('plugins/order/app/controllers/admin/catalog_post.php');
            } else {
                $_view['entry'] = $post['entry'];
    ...
    if (empty($_GET['id'])) {
        ...
        $_view['entry']['code']      = $GLOBALS['plugin']['order']['setting']['default_code'] ? localdate($GLOBALS['plugin']['order']['setting']['default_code']) : '';
        $_view['entry']['text_type'] = $GLOBALS['plugin']['order']['setting']['text_type'];
    } else {
        $entries = model('select_entries', [
            'where' => [
                'entries.id = :id AND types.code = ' . db_escape('catalog'),
                [
                    'id' => $_GET['id'],
                ],
            ],
