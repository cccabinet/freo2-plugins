    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 mb-2 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 mb-2">
            ...
            <nav style="--bs-breadcrumb-divider: '>';">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="<?php t(MAIN_FILE) ?>/admin/gallery">ギャラリー管理</a></li>
        ...
        <div class="card shadow-sm mb-3">
            ...
            <?php else : ?>
                ...
                <form action="<?php t(MAIN_FILE) ?>/admin/gallery_form<?php $_view['entry']['id'] ? t('?id=' . $_view['entry']['id']) : '' ?>" method="post" class="register validate">
                    ...
                    <div class="card shadow-sm mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group mb-2">
                                <label class="fw-bold">コード <span class="badge text-light bg-secondary" data-toggle="tooltip" title="ギャラリーのURLに使用されます。">？</span> <span class="badge bg-danger">必須</span></label>
                            ...
                            <?php if ($GLOBALS['plugin']['gallery']['setting']['text_type'] !== 'none') : ?>
                            <div class="form-group mb-2">
                                <label class="fw-bold">本文（<a href="#" data-bs-toggle="modal" data-bs-target="#mediaModal">メディア</a>）</label>
                                <textarea name="text" rows="10" cols="50" class="form-control<?php if ($GLOBALS['plugin']['gallery']['setting']['text_type'] === 'wysiwyg') : ?> editor<?php endif ?>"><?php t($_view['entry']['text']) ?></textarea>
                            </div>
                            <?php endif ?>
                            <?php if ($GLOBALS['plugin']['gallery']['setting']['use_pictures']) : ?>
                            <div class="form-group mb-2">
                            ...
                            <?php endif ?>
                            <?php if ($GLOBALS['plugin']['gallery']['setting']['use_thumbnail']) : ?>
                            <div class="form-group mb-2">
                ...
                <?php if ($GLOBALS['plugin']['gallery']['setting']['use_approve'] && $GLOBALS['authority']['power'] >= 3) : ?>
                <form action="<?php t(MAIN_FILE) ?>/admin/gallery_approve" method="post" class="approve">
                    <input type="hidden" name="_token" value="<?php t($_view['token']) ?>" class="token">
                    <input type="hidden" name="id" value="<?php t($_view['entry']['id']) ?>">
                ...
                <?php endif ?>
                <form action="<?php t(MAIN_FILE) ?>/admin/gallery_delete" method="post" class="delete">
                    <input type="hidden" name="_token" value="<?php t($_view['token']) ?>" class="token">
                    <input type="hidden" name="id" value="<?php t($_view['entry']['id']) ?>">
                    <div class="card shadow-sm mb-3">
                        <div class="card-header">
